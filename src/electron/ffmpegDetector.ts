 ...
 * 4. System PATH
 */
export async function detectFFmpeg(): Promise<FFmpegPathResult | null> {
  if (cachedResult) {
    return cachedResult;
  }
 ...
 */
export function getFFmpegPath(): FFmpegPathResult | null {
  return cachedResult;
}
 ...
 */
export async function getFFmpegVersion(executablePath: string): Promise<string | null> {
  return new Promise((resolve) => {
