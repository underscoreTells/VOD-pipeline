...
describe("naming model catalog", () => {
  ...
  it("maps naming models to providers", () => {
    ...
    expect(getNamingModelProvider("gemini-3-flash-preview")).toBe("gemini");
    expect(getNamingModelProvider("kimi-k2.5")).toBe("kimi");
  });
  ...
  it("normalizes deprecated naming model values", () => {
    ...
    expect(normalizeNamingModel("gemini-1.5-flash")).toBe("gemini-3-flash-preview");
    expect(normalizeNamingModel("gemini-3.0-flash")).toBe("gemini-3-flash-preview");
    expect(normalizeNamingModel("gemini-3-flash")).toBe("gemini-3-flash-preview");
